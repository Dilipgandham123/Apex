...
const tickerItems = ["LEARN TO DRIVE","RTO ASSISTANCE","MANUAL & AUTOMATIC CARS","WOMEN INSTRUCTORS","FREE TRIAL CLASS","LICENCE ASSISTANCE"];
...
export default function Home(){
  ...
  const submitTrial=async(event:FormEvent<HTMLFormElement>)=>{
    ...
    const details=[
      ...
      "Please share the available trial-class timing. Thank you.",
    ].filter(Boolean).join("\n");
    window.open(`https://wa.me/918106373266?text=${encodeURIComponent(details)}`,"_blank","noopener,noreferrer");
    form.reset();
  ...
  return <main id="main-content">
    ...
    <header className="header">
      <a className="logo" href="#top" onClick={close}><strong>SRI SAI <em>ANU</em></strong><span>MOTOR DRIVING SCHOOL</span></a>
      <nav className="desktop-nav" aria-label="Primary navigation">{nav.map(([label,href])=><a key={label} href={href}>{label}</a>)}<a className="primary-link" href="#booking">BOOK FREE TRIAL</a></nav>
      ...
      <nav ref={mobileMenuRef} id="mobile-menu" aria-label="Mobile navigation" aria-hidden={!menuOpen} className={menuOpen?"mobile-nav open":"mobile-nav"}>{nav.map(([label,href])=><a onClick={close} key={label} href={href}>{label}</a>)}<a onClick={close} className="primary-link" href="#booking">BOOK FREE TRIAL</a></nav>
    </header>
    ...
    <section id="top" className="hero">
      ...
      <HeroScrollFade className="hero-inner"><Stagger className="hero-sequence" delay={0.08} interval={0.07} amount={0.05}>
        ...
        <StaggerItem className="hero-lower">
          ...
          <div className="actions"><AnimatedCTA className="button" href="#booking">BOOK FREE TRIAL</AnimatedCTA><AnimatedCTA className="button ghost" href="#courses">SEE COURSES</AnimatedCTA></div>
        </StaggerItem>
    ...
    <section id="booking" className="section booking">
      <div className="shell booking-grid"><Reveal><div><p className="kicker">FREE TRIAL CLASS</p><h2>FIRST CLASS<br/>ON US.</h2><p className="intro">Thirty minutes, dual-control car, zero obligation. Tell us when suits you and an instructor will call within 2 working hours.</p><a className="contact" href="tel:+918106373266">+91 81063 73266</a><p className="address">2-72/1, Indian Oil petrol pump line, opp. State Bank of India, Megha Hills, Sri Sai Nagar, Madhapur, Hyderabad 500081</p><a className="direction" href="https://maps.app.goo.gl/Pd3zQ4Q6145z1KoQ7">GET DIRECTIONS <Arrow/></a></div></Reveal>
        <Reveal delay={.08}><form aria-label="Book a free trial class on WhatsApp" onSubmit={submitTrial}><label htmlFor="trial-name"><span>FULL NAME</span><input id="trial-name" name="name" autoComplete="name" placeholder="Full name" required/></label><label htmlFor="trial-phone"><span>MOBILE NUMBER</span><input id="trial-phone" name="phone" type="tel" inputMode="tel" autoComplete="tel" pattern="[0-9+() -]{10,18}" placeholder="Mobile number (10 digits)" required/></label><div className="form-row"><label htmlFor="trial-course"><span>COURSE</span><select id="trial-course" name="course">{courses.map(c=><option key={c.name}>{c.name[0]+c.name.slice(1).toLowerCase()} — {c.price}</option>)}</select></label><label htmlFor="trial-slot"><span>PREFERRED SLOT</span><select id="trial-slot" name="slot"><option>Morning (6-9 AM)</option><option>Afternoon (12-4 PM)</option><option>Evening (4-7 PM)</option></select></label></div><label htmlFor="trial-notes"><span>NOTES <small>(OPTIONAL)</small></span><textarea id="trial-notes" name="notes" placeholder="Anything we should know?"/></label><button className="button whatsapp-submit" type="submit"><WhatsappLogo size={22} weight="bold" aria-hidden="true"/><span>{formSubmitted?"MESSAGE READY — OPEN AGAIN":"CONTINUE ON WHATSAPP"}</span></button><small id="trial-status" className={`form-status ${formSubmitted?"success":"idle"}`} role="status" aria-live="polite">{formSubmitted?"Your personalized message is ready. Tap Send in WhatsApp to confirm the trial request.":"We do not store these details. They are used only to prepare your WhatsApp message."}</small></form></Reveal>
      </div>
    </section>

    <footer><div className="shell"><div className="footer-word">SRI SAI ANU</div><div className="footer-grid"><div><h3>REACH US</h3><a href="tel:+918106373266">+91 81063 73266</a><p>2-72/1, Indian Oil petrol pump line, opp. State Bank of India, Megha Hills, Sri Sai Nagar, Madhapur, Hyderabad 500081</p><a href="https://maps.app.goo.gl/Pd3zQ4Q6145z1KoQ7">GET DIRECTIONS <Arrow/></a></div><div><h3>HOURS</h3><p>Open daily: 6 AM – 7 PM</p><p>Training slots are available at convenient timings.</p></div></div><div className="copyright"><span>© 2026 SRI SAI ANU MOTOR DRIVING SCHOOL, MADHAPUR, HYDERABAD</span><span>BUILT FOR THE ROAD.</span></div></div></footer>
